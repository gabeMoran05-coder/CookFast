...
function useDragVars() {
  ...
  const startDrag = (event) => {
    ...
    const start = { x: pointer.clientX, y: pointer.clientY, ...offset };

    const move = (moveEvent) => {
      const nextPointer = moveEvent.touches?.[0] ?? moveEvent;
      setOffset({
        x: start.x + nextPointer.clientX - start.clientX,
        y: start.y + nextPointer.clientY - start.clientY,
      });
    };
    const stop = () => {
...
export default function App() {
  ...
  const ingredients = useMemo(() => {
    return selectedIngredients.length ? selectedIngredients : ['tomate', 'cebolla', ...meal.ingredients];
  }, [selectedIngredients, meal]);
  ...
  const toggleIngredient = (ingredient) => {
    setSelectedIngredients((current) => current.includes(ingredient) ? current.filter((item) => item !== ingredient) : [...current, ingredient]);
  };
    ...
    <main className="app" style={sceneStyle}>
      <section className="heroPanel">
        ...
        <div className="statusGrid">
          <div><span>Zona elegida</span><strong>{selectedCountry.label}</strong></div>
          <div><span>Momento</span><strong>{meal.label}</strong></div>
          <div><span>Ingredientes activos</span><strong>{ingredients.slice(0, 5).join(', ')}{ingredients.length > 5 ? ` +${ingredients.length - 5}` : ''}</strong></div>
        </div>
      </section>

      <section className="globeShell" aria-label="Globo interactivo">
        <Globe countries={COUNTRY_POINTS} selectedCountry={selectedCountry} meal={meal} onSelectCountry={setSelectedCountry} />
        <div className="mapHint"><Rotate3D size={18} /> Arrastra para rotar. Toca un punto para elegir país.</div>
      </section>

      <aside className="mealSlider" aria-label="Selector de comida">
        <div className="mealIcon">{meal.darkness > 0.4 ? <Moon size={20} /> : <SunMedium size={20} />}</div>
        <input aria-label="Cambiar comida del día" type="range" min="0" max={MEALS.length - 1} step="1" value={mealIndex} onChange={(event) => setMealIndex(Number(event.target.value))} />
        <div className="mealReadout"><strong>{meal.label}</strong><span>{String(meal.hour).padStart(2, '0')}:00</span></div>
        <div className="mealTicks">{MEALS.map((item, index) => <button key={item.id} className={index === mealIndex ? 'active' : ''} onClick={() => setMealIndex(index)} title={item.label}>{item.short}</button>)}</div>
      </aside>

      <button className={countryPanelOpen ? 'drawerToggle countries isOpen' : 'drawerToggle countries'} onClick={() => setCountryPanelOpen((value) => !value)} aria-label={countryPanelOpen ? 'Ocultar países' : 'Abrir países'}>
        {countryPanelOpen ? <ChevronDown size={22} /> : <ChevronUp size={22} />}
      </button>

      <aside className={countryPanelOpen ? 'countryDock open' : 'countryDock'} aria-label="Selector de país" style={countryDrag.style}>
        <div className="dockHeader dragHandle" onMouseDown={countryDrag.startDrag} onTouchStart={countryDrag.startDrag}><GripVertical size={15} /><Compass size={16} /> País / cocina</div>
      ...
      <button className={ingredientPanelOpen ? 'drawerToggle ingredients isOpen' : 'drawerToggle ingredients'} onClick={() => setIngredientPanelOpen((value) => !value)} aria-label={ingredientPanelOpen ? 'Ocultar ingredientes' : 'Abrir ingredientes'}>{ingredientPanelOpen ? <ChevronLeft size={22} /> : <ChevronRight size={22} />}</button>

      <aside className={ingredientPanelOpen ? 'ingredientDrawer open' : 'ingredientDrawer'} style={ingredientDrag.style}>
        <div className="drawerTitle dragHandle" onMouseDown={ingredientDrag.startDrag} onTouchStart={ingredientDrag.startDrag}><div><p className="eyebrow">Catálogo mundial curado</p><h2>Ingredientes</h2></div><GripVertical size={22} /></div>
        <p className="sourceNote">{CATALOG_NOTE}</p>
        <label className="searchBox"><Search size={16} /><input value={ingredientSearch} onChange={(event) => setIngredientSearch(event.target.value)} placeholder="Buscar ingrediente o bebida" /></label>
      ...
      <button className={recipesOpen ? 'drawerToggle recipes isOpen' : 'drawerToggle recipes'} onClick={() => setRecipesOpen((value) => !value)} aria-label={recipesOpen ? 'Ocultar recetas' : 'Abrir recetas'}>{recipesOpen ? <ChevronRight size={22} /> : <ChevronLeft size={22} />}</button>

      <section className={recipesOpen ? 'recipePanel open' : 'recipePanel'} style={recipeDrag.style}>
        <div className="panelTitle dragHandle" onMouseDown={recipeDrag.startDrag} onTouchStart={recipeDrag.startDrag}><div><p className="eyebrow">{selectedCountry.cuisine || 'sin filtro de país'}</p><h2>{meal.label} para {selectedCountry.label.toLowerCase()}</h2></div><GripVertical size={22} /></div>
        {loading && <p className="muted">Buscando recetas...</p>}
